...
public class MeasurementListNodeCleanerTest {
  @Test
  public void testApply() throws Exception {
    // Input tree
    // Data from: http://www.itl.nist.gov/div898/handbook/eda/section3/eda35h3.htm
    final List<Measurement> list = Arrays.asList(
    ...
    final List<Measurement> expected = Arrays.asList(
        new Measurement(1.0d, -250000.0d), new Measurement(1.0d, 0680000.0d), new Measurement(1.0d, 0940000.0d), new Measurement(1.0d, 1150000.0d),
        new Measurement(1.0d, 1200000.0d), new Measurement(1.0d, 1260000.0d), new Measurement(1.0d, 1260000.0d), new Measurement(1.0d, 1340000.0d),
        new Measurement(1.0d, 1380000.0d), new Measurement(1.0d, 1430000.0d), new Measurement(1.0d, 1490000.0d), new Measurement(1.0d, 1490000.0d),
        new Measurement(1.0d, 1550000.0d), new Measurement(1.0d, 1560000.0d), new Measurement(1.0d, 1580000.0d), new Measurement(1.0d, 1650000.0d),
    ...
    final MergeableList<Measurement> measurementList = new MergeableList<>(list);
    final MergeableNode<MergeableList<Measurement>> measurementListNode = new MergeableNode<>("root", measurementList);
    final MergeableTree<MergeableList<Measurement>> measurementListTree = new MergeableTree<>(measurementListNode);
    final MergeableList<Measurement> actualMeasurement = measurementListTree.map(MergeableTree::new, new MeasurementListNodeCleaner<>(10)).getRootNode().getData();

    assertEquals("Expected correct size", list.size() - 3, actualMeasurement.getList().size(), 0.0000001d);
    assertThat("Expected correct list", actualMeasurement.getList(), is(expected));
  }
}
