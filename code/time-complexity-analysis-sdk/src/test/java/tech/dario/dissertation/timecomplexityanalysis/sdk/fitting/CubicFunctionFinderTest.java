...
public class CubicFunctionFinderTest {
  ...
  public void findFittingFunction() throws Exception {
    ...
    assertEquals("Expected correct string representation", "2.000000 * n^3 + 6.000000 * n^2 + 1.000000 * n + 8.000000 [rms: 0.000000]", fittingFunctionOptional.get().toString());

    long minN = 0L;
    long maxN = 30L;
    XYSeriesCollection dataset = new XYSeriesCollection();
    XYSeries currentSeries = new XYSeries(fittingFunctionOptional.get().getClass().getSimpleName());
    for(double n = minN; n < maxN; n += (maxN - minN) / 200.0d) {
      currentSeries.add(n, fittingFunctionOptional.get().f(n));
    }
