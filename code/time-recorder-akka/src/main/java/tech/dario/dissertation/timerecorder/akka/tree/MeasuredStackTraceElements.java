...
public class MeasuredStackTraceElements {
    ...
    private static Map<String, Boolean> measuredStackTraceElementNamesCache = new HashMap<String, Boolean>();
    ...
    public static MeasuredStackTraceElements fromStackTrace(StackTraceElement[] stackTrace) throws NotFoundException {
        List<String> measuredStackTraceElementNames = new ArrayList<String>(stackTrace.length);
        for (StackTraceElement stackTraceElement : stackTrace) {
            String stackTraceElementName = getStackTraceElementName(stackTraceElement);
            Boolean cached = measuredStackTraceElementNamesCache.get(stackTraceElementName);
            if (cached != null) {
                if (cached) {
                    measuredStackTraceElementNames.add(stackTraceElementName);
                }
            } else {
                boolean measuredMethod = isMeasuredMethod(stackTraceElement);
    ...
    public static MeasuredStackTraceElements empty() {
        return new MeasuredStackTraceElements(Collections.<String>emptyList());
    }
    ...
    public boolean equals(Object o) {
        ...
        if (stackTraceElementNames != null && that.stackTraceElementNames != null) {
            return stackTraceElementListsAreEqual(stackTraceElementNames, that.stackTraceElementNames);
        } else {
